                ...
                <section key={lane.label} className={`rounded-2xl border-2 ${lane.tint} overflow-hidden shadow-warm`}>
                  ...
                  <div className="p-4 grid gap-5" style={{ gridTemplateColumns: showCompleted[lane.label] ? 'minmax(0,1fr) minmax(0,1fr) minmax(0,1fr)' : 'minmax(0,1fr) minmax(0,1fr) auto' }}>
                    <div>
                      ...
                      <div className="grid gap-3 items-start [grid-template-columns:repeat(auto-fill,minmax(180px,1fr))]">{lane.data.pending.map(renderOrderCard)}</div>
                    </div>
                    <div>
                      <h2 className="font-serif-tc font-bold text-foreground mb-3 flex items-center gap-2">
                      ...
                      <div className="grid gap-3 items-start [grid-template-columns:repeat(auto-fill,minmax(180px,1fr))]">{lane.data.inProgress.map(renderOrderCard)}</div>
                    </div>
                    <div className={showCompleted[lane.label] ? '' : 'w-auto'}>
                      ...
                      {showCompleted[lane.label] && (
                        <div className="grid gap-3 items-start [grid-template-columns:repeat(auto-fill,minmax(180px,1fr))]">{lane.data.completed.map(renderOrderCard)}</div>
                      )}
                    </div>
                  </div>
                </section>
